...
def change_area(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_la_no_null["Living area"] = df_la_no_null.apply(
        lambda col: fill_living_area(col), axis=1
    )
...
def categorical_encoding(data_no_multicollinearity: pd.DataFrame) -> pd.DataFrame:
    ...
    post_code_stat = data_no_multicollinearity["Post code"].value_counts(
        ascending=False
    )
...
def categorical_encoding_client(
    big_dataframe: pd.DataFrame, small_dataframe: pd.DataFrame
) -> pd.DataFrame:
    # ### Categorical data encoding
    df = small_dataframe
